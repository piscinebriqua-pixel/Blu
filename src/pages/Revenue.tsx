...
const Revenue: React.FC = () => {
    ...
    const fetchRevenueData = async () => {
        try {
            ...
            // 4. Fetch Pending (Positive balances in clients)
            ...
            const pendingTotal = clients?.reduce((acc: number, c: any) => acc + (c.balance || 0), 0) || 0;
        ...
        >
            <div className="flex flex-col gap-6">
                ...
                <div className="flex bg-white dark:bg-slate-800 p-1 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm self-start">
                    {(['day', 'week', 'month', 'year'] as const).map((range) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${timeRange === range
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="card-premium vibrant grad-blue p-6 relative overflow-hidden group">
                    ...
                    <div className="card-white p-6 border-blue-100/50 dark:border-blue-800/30">
                    ...
                    <div className="card-white p-6 border-emerald-100/50 dark:border-emerald-800/30">
                    ...
                    <div className="card-white p-6 border-rose-100/50 dark:border-rose-800/30">
                        <p className="text-premium-label text-rose-600 dark:text-rose-400 mb-1">Reste à Percevoir</p>
                        <h3 className="text-3xl font-black text-slate-800 dark:text-white tracking-tighter">
                            {Math.max(0, stats.realizedTotal - stats.total).toLocaleString()} <span className="text-sm opacity-40">DT</span>
                        </h3>
                        <div className="flex items-center gap-2 mt-2">
                            <Calendar size={14} className="text-rose-400" />
                            <span className="text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase">
                                Taux: {stats.realizedTotal > 0 ? ((stats.total / stats.realizedTotal) * 100).toFixed(0) : 0}%
                            </span>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {/* Payment Methods */}
                    <div className="card-white !p-8 flex flex-col gap-6 lg:col-span-1">
                    ...
                    <div className="card-white !p-8 flex flex-col gap-6 lg:col-span-2">
                        <div className="flex items-center justify-between">
                            <h4 className="text-[12px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
