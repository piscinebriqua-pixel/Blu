        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl w-full max-w-md max-h-[80vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h3 className="text-sm font-black uppercase tracking-widest text-slate-800">Sélectionner un technicien</h3>
                    <button onClick={onClose} className="p-2 hover:bg-slate-200 rounded-full transition-colors" title="Fermer">
                ...
                <div className="p-4 border-b border-slate-100 bg-white">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all"
                            value={search}
                    ...
                    {loading ? (
                        ...
                        filtered.map(tech => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-black mr-4 uppercase shrink-0 ${isSelected ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-blue-100 group-hover:text-blue-600'
                                        }`}>
                                        {tech.photo_url ? (
                                    ...
                                    <div className="text-left flex-1">
                                        <p className={`text-sm font-black ${isSelected ? 'text-blue-900' : 'text-slate-700'}`}>{tech.full_name}</p>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Technicien</p>
                                    </div>
                                    {isSelected && (
